...
class AskColor(customtkinter.CTkToplevel):
    ...
    def __init__(self,
        ...
        # Ensure window appears on top and is modal
        self.lift()
        ...
        self.color_entry = customtkinter.CTkEntry(master=self.frame, height=50,corner_radius=self.corner_radius,textvariable=tkinter.StringVar(), fg_color=self.default_hex_color)
        self.color_entry.pack(fill="both", padx=10)
        self.color_entry.bind("<Return>", self.update_from_entry)
        self.color_entry.bind("<FocusOut>", self.update_from_entry)  # Actualizar el color cuando el Entry pierde el foco
        self.color_entry.configure(justify="center")# Agregar esta línea para capturar el evento en toda la aplicación
    ...
    def _make_modal(self):
        """Called after window is fully rendered to safely grab focus."""
        try:
            self.attributes("-topmost", True)
            self.focus_force()
            self.grab_set()
    ...
    def update_colors(self):
        ...
        self.color_entry.insert(0, self.default_hex_color)
        self.color_entry.configure(fg_color=self.default_hex_color)
        
        self.label.configure(text=str(self.default_hex_color))
        
        if self.brightness_slider_value.get() < 70:
            self.label.configure(text_color="white")
            self.color_entry.configure(text_color="white")
        else:
            self.label.configure(text_color="black")
            self.color_entry.configure(text_color="black")
    ...
    def update_from_entry(self, event=None):
        entered_color = self.color_entry.get()
        if entered_color.startswith("#") and len(entered_color) == 7:
            self.set_initial_color(entered_color)
            self.update_colors()
